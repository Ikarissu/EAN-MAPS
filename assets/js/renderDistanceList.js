...
function renderDistanceList(opts = {}) {
  ...
  const createPointHTML = (pointInfo) => {
    ...
    const formattedCoords = `${pointInfo.lat.toFixed(
      6
    ...
    distanceList.insertAdjacentHTML(
      ...
      `<div class="distance-info" data-record-index="${globalIndex}">
                <div class="mode-header">
                  ...
                  <span class="mode-text">${r.typeLabel || r.type}: ${
        r.distance
      } Kilómetros</span>
                </div>
                <div>
                  ...
                  <p class="start-hour">Hora de salida (${r.tzLabel}): ${
        r.start_hour
      }</p>
                  <p class="end-hour">Hora de llegada aproximada (${
                    r.tzLabel
                  }): ${r.end_hour}</p>
